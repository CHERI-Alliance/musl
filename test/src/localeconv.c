...
int main (int argc, char *argv[])
{
    ...
    case '1':// test reading char
        ...
        if (((perm & __CHERI_CAP_PERMISSION_READ__) == 0) || (local->int_frac_digits != CHAR_MAX)) return 1;
#else
...
#ifdef __riscv_zcheripurecap
        if ((perm & __CHERI_CAP_PERMISSION_WRITE__) != 0) return 1;
#else
    ...
    case '3':// test reading string
      ...
      if (((perm & (__CHERI_CAP_PERMISSION_READ__ |
           ...
           (__CHERI_CAP_PERMISSION_READ__ |
            __CHERI_CAP_PERMISSION_CAPABILITY__)) ||
          (strcmp(local->decimal_point, ".") != 0))
        return 1;
#else
      if (((perm & __CHERI_CAP_PERMISSION_PERMIT_LOAD_CAPABILITY__) == 0) ||
          (strcmp(local->decimal_point, ".") != 0))
        return 1;
#endif
      break;
